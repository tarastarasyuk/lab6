...
MODULE_PARM_DESC(num_of_greet, "Number of times 'Hello, World!' to be printed");
...
static int __init hello_init(void)
{
    if (num_of_greet == 0 || (num_of_greet >= 5 && num_of_greet <= 10)) {
        printk(KERN_WARNING "Parameter is 0 or is in range between 5 and 10");  
    }
